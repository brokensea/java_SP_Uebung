...
public class Bootsverleih {
    ...
    public Boot geringstenBootanhandAnzahlReservierungen() {
        if (boote.isEmpty()) {
            System.out.println("Keine Boote verfügbar.");
            return null;
        }

        Boot nichtbeliebtBoot = boote.get(0);
        int minReservations = nichtbeliebtBoot.getReservations().size();

        // Find the maximum reservations count
        for (Boot boot : boote) {
            int reservationsCount = boot.getReservations().size();
            if (reservationsCount < minReservations) {
                minReservations = reservationsCount;
                nichtbeliebtBoot = boot;
            }
        }
        System.out.println("geringsten Boot ID ist: " + nichtbeliebtBoot.getId());

        // Return the most popular boot
        return nichtbeliebtBoot;
    }
